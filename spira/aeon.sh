...
# THE GUARD IS BOUND TO $WORK, NOT TO $SPIRA_REPO. An aeon works inside its own
# worktree; if it left tracked files modified or staged-but-uncommitted there, the close
# is refused: unreviewed changes in the working tree are invisible to the commit graph.
#
# THE SHARED CHECKOUT ($SPIRA_REPO) IS NOT THIS GUARD'S SCOPE. An aeon cannot control
# what other sessions leave there, and binding the guard to a global condition punishes
# whichever aeon happens to close next for a condition it neither caused nor can fix —
# an unbounded requeue loop by construction. The shared checkout is a box property;
# skew.sh reports DIRTY on a timer and escalates once. One stray file in the shared
# checkout must not stop the queue from draining.
#
# DOES NOT FIRE ON A BEAD THAT WAS REOPENED ABOVE. st="open" from the no-commit check or
# the delivers check means the guard below is skipped.
#
...
if [ "$st" = "closed" ] && [ "$committed" = "yes" ] && [ -z "${SPIRA_ALLOW_PROD_DIRTY:-}" ]; then
    _wt_dirty="$(git -C "$WORK" status --porcelain --untracked-files=no 2>/dev/null)"
    if [ -n "$_wt_dirty" ]; then
        _wt_note="Reopened by aeon.sh: bead closed while $WORK carried uncommitted tracked modifications. Unreviewed changes in the worktree are invisible to the commit graph. Stage and commit every file this bead changed, or restore paths not intended to be modified:

    git -C $WORK restore -- <path>

Modified paths:
$(printf '%s\n' "$_wt_dirty" | sed 's/^/  /')

Override (only when the modification is intentional and will be handled separately):
    SPIRA_ALLOW_PROD_DIRTY=1"
        bead_reopen "$BEAD_ID" "$_wt_note"
        ...
        st="open"
        log "$FAYTH: $BEAD_ID REOPENED — own worktree dirty: $(git -C "$WORK" status --short --untracked-files=no 2>/dev/null | head -5 | tr '\n' ' ')"
        REQUEUE_CAUSE="prod-dirty"
        REQUEUE_WHY="Bead closed while $WORK carried uncommitted tracked modifications. Stage and commit (or restore) before this bead can close."
        unset _wt_dirty _wt_note
    fi
    unset _wt_dirty
fi
