...
#
# WHY THIS IS NOT A `test-*.sh`. The landing gate runs every suite on every branch, and a
# browser is not something a clone has any reason to have — a gated suite that skips on most
# machines trains everyone to ignore its skip line. `test-loom-page.sh` gates the half that needs
# no browser: the view model, and the structural properties of the page. This is the other
# half, run by hand when the page changes.
#
